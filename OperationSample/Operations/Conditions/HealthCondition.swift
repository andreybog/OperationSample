...
struct HealthCondition: OperationCondition {
    ...
    func evaluateForOperation(_ operation: BaseOperation, completion: @escaping (OperationConditionResult) -> Void) {
        ...
        if !unauthorizedShareTypes.isEmpty {
            failed(unauthorizedShareTypes, completion: completion)
        }
        else {
    ...
    private func failed(_ unauthorizedShareTypes: Set<HKSampleType>, completion: (OperationConditionResult) -> Void) {
 ...
 */
private class HealthPermissionOperation: BaseOperation {
    let shareTypes: Set<HKSampleType>
    let readTypes: Set<HKSampleType>
