...
class EarthquakesTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        let operation = LoadModelOperation { context in
            ...
            DispatchQueue.main.async {
                let request = NSFetchRequest<Earthquake>(entityName: Earthquake.enetityName)
                
                request.sortDescriptors = [NSSortDescriptor(key: "timestamp", ascending: false)]
                
                request.fetchLimit = 100
                
                let controller = NSFetchedResultsController<Earthquake>(fetchRequest: request, managedObjectContext: context, sectionNameKeyPath: nil, cacheName: nil)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "earthquakeCell", for: indexPath) as! EathquakeTableViewCell
        
        if let earchquake = fetchedResultsController?.object(at: indexPath) {
            cell.configure(earchquake)
        }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        guard let navigationVC = segue.destination as? UINavigationController,
            let detailVC = navigationVC.viewControllers.first as? EarthquakeTableViewController
        else {
            return
        }
    ...
    private func updateUI() {
        do {
            ...
            print("Error in the fetched results controller: \(error as NSError)")
        }
    ...
    private func getEarthquakes(_ userInitiated: Bool = true) {
        if let context = fetchedResultsController?.managedObjectContext {
            let getEarthquakeOperation = GetEarthquakesOperation(context: context) {
                DispatchQueue.main.async {
            ...
            getEarthquakeOperation.userInitiated = userInitiated
            operationQueue.addOperation(getEarthquakeOperation)
        }
        else {
            /*
