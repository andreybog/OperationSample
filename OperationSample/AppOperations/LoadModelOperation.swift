...
class LoadModelOperation: BaseOperation {
    ...
    override func execute() {
        ...
        let cachesFolder = try! FileManager.default.url(for: .cachesDirectory,
                                                        in: .userDomainMask,
                                                        appropriateFor: nil,
                                                        create: true)
        ...
        if persistentStoreCoordinator.persistentStores.isEmpty {
            if let error = error {
                print("Erorr creating SQLite store: \(error).")
            }
    ...
    private func createStore(_ persistentStoreCoordinator: NSPersistentStoreCoordinator, atURL URL: URL?, type: String = NSSQLiteStoreType) -> NSError? {
        var error: NSError?
    ...
    override func finished(_ errors: [NSError]) {
        ...
        alert.message = "An error occured while loading the databse. \(firstError.localizedDescription). Please try again later."
